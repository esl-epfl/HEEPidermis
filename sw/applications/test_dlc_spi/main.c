...
spi_host_t* spi_device = spi_flash;
uint8_t src_slot = DMA_TRIG_SLOT_SPI_FLASH_RX;
...
####### CONFIGURE THE SPI TRANSFER ##########################*/
    ...
    if( spi_host_init(spi_device, 1)!= SPI_FLAG_SUCCESS) return EXIT_FAILURE;
