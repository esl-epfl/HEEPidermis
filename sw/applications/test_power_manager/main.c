...
int main(int argc, char *argv[])
{
    ...
    // THERE IS NO POWER GATING ON THE CPU
    // BUT WE ARE TRYING THAT FUNCTIONALITY IS KEPT
    ...
    // THERE IS NO POWER GATING ON THE PERIPHERAL SUBSYSTEM
    // BUT WE ARE TRYING THAT FUNCTIONALITY IS KEPT
    ...
    for(uint32_t i = 0; i < EXTERNAL_DOMAINS; ++i) {
        ...
        if( i==0 ) 
            PRINTF("TESTING CARUS\n\r");
        else if (i == 1)
            PRINTF("TESTING CGRA\n\r");
        else{
            PRINTF("ERROR EXTERNAL DOMAIN NUMBER!\n\r");
            exit(-1);
        }
        ...
        if (power_gate_external(&power_manager, i, kOff_e, &power_manager_counters) != kPowerManagerOk_e)
        {
            PRINTF("Error: power manager fail.\n\r");
            return EXIT_FAILURE;
        }
        ...
        if (power_gate_external(&power_manager, i, kOn_e, &power_manager_counters) != kPowerManagerOk_e)
        {
            PRINTF("Error: power manager fail.\n\r");
            return EXIT_FAILURE;
        }
        ...
        // ------------ set retentive domain ------------
        if (power_gate_counters_init(&power_manager_counters, 0, 0, 0, 0, 0, 0, 30, 30) != kPowerManagerOk_e)
        {
            PRINTF("Error: power manager fail. Check the reset and powergate counters value\n\r");
            return EXIT_FAILURE;
        }
        // Set retention mode on for external domain block i
        if (power_gate_external(&power_manager, i, kRetOn_e, &power_manager_counters) != kPowerManagerOk_e)
        {
            PRINTF("Error: power manager fail.\n\r");
            return EXIT_FAILURE;
        }
        // Wait some time
        for (int k=0; k<100; k++) asm volatile("nop");
        // Set retention mode off for external domain block i
        if (power_gate_external(&power_manager, i, kRetOff_e, &power_manager_counters) != kPowerManagerOk_e)
        {
            PRINTF("Error: power manager fail.\n\r");
            return EXIT_FAILURE;
        }
